...
use crate::pos::spot::spotops_types::{
    self, add_item_errors, delete_item_errors, spot_ops_types, AddInvoiceOp, AddItemOp,
    DeleteItemOp,
};
...
impl spotops_types::AddItemOp {
    ...
    pub fn create_add_item_op(
        ...
        if full_invoice_number.trim().is_empty() {
            return Err(add_item_errors::EmptyField {
...
impl spotops_types::DeleteItemOp {
    ...
    pub fn create_delete_item_op(
        full_invoice_number: &str,
        item_id: &str,
    ) -> Result<DeleteItemOp, delete_item_errors> {
        if full_invoice_number.trim().is_empty() {
            return Err(delete_item_errors::EmptyField {
                field: "full_invoice_number",
            });
        }
        ...
        Ok(DeleteItemOp {
            op_type: spot_ops_types::DeleteItem,
            full_invoice_number: full_invoice_number.to_string(),
            item_id: item_id.to_string(),
        })
    }
