...
pub fn get_operator_averaged_stats_start_end(
    start_date: DateTime<Utc>,
    end_date: DateTime<Utc>,
) -> Result<Vec<OperatorStats>, String> {
    ...
    let sessions = sessions_repo::get_sessions_start_end(
        &mut conn,
    ...
    let mut operator_map: std::collections::HashMap<i32, (i64, i64)> =
    ...
    for (user_id, (total_garments, total_tickets)) in operator_map {
        let user = users_repo::get_user_by_id(&mut conn, user_id).map_err(|e| e.to_string())?;
        let avg_garments = (total_garments as f64 / total_hours).round() as i64;
        let avg_tickets = (total_tickets as f64 / total_hours).round() as i64;
